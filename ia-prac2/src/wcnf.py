...
class WCNFFormula(object):
    ...
    def casillegas(self, list):
        """Entra una clausula hard de cualquien numero de atributos
            y devuelve una clausula hard de 3 atributos.
        """
        if len(list) ==2:
            self.add_clause([list[0], list[1], list[1]], TOP_WEIGHT)
        else:
            self.add_clause([list[0], list[0], list[0]], TOP_WEIGHT)

    
    def tepasas(self, list):
        """Entra una clausula hard de cualquien numero de atributos
            y devuelve una lsita de clausulas hard de 3 atributos.
        """
        global NUMOFZ
        try:
            if len(list) < 3:
                self.casillegas(list)
            elif len(list) > 3:
                NUMOFZ+=1
                z=NUMOFZ
                self.new_var()
                self.add_clause([list[0], list[1], z], TOP_WEIGHT)
                list.append(-z)
                self.tepasas(list[2:])
            else:
                self.add_clause(list, TOP_WEIGHT)
        except:
            print("Error: object has NoneType")

    def to_13wpm(self):
        global NUMOFB
        global NUMOFZ
        """Generates a new formula that is the 1,3-WPM 2equivalent
        of this one."""
        formula13 = WCNFFormula()
        hards = [] # Item format: [literals]
        for _ in self.soft:
            formula13.new_var()
        #add soft clauses to formula13
        NUMOFB=self.num_vars
        for i in self.soft:
            if len(i[1])>1:
                NUMOFB+=1
                b = NUMOFB
                formula13.new_var()
                formula13.add_clause([-b], i[0]) #i[0] añade el peso.
                i[1].append(b)
                hards.append(i[1])
            else:
                formula13.new_var()
                formula13.add_clause(i[1], i[0])
        NUMOFZ=NUMOFB
        #add self hard clauses to formula13
        for i in self.hard:
            hards.append(i)
        #convert hard clauses to 3
        for i in hards:
            try:
                if len(i)<3:
                    formula13.casillegas(i)
                elif len(i)>3:
                    formula13.tepasas(i)
                else:
                    formula13.add_clause(i, TOP_WEIGHT)
            except:
                print("Error: object has NoneType")
        return formula13
